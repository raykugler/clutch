...
class Modular_Helmet_List extends React.Component {
    constructor(props){
        super(props)
        this.state={modularstate: 'modular'
    }
}
    ...
    render(clutches){
        ...
        {({loading, error, data}) =>{
           ...
           return data.clutches.filter(clutch => clutch[this.state.modularstate]
        ).map(({ price, sku, size, helmetType, description, image, brand, name, id,color, helmetColors }) => (
...
export default Modular_Helmet_List;
