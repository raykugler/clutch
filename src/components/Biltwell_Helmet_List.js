...
class Biltwell_Helmet_List extends React.Component {
    constructor(props){
        super(props)
        this.state={biltwellstate: 'biltwell'
    }
}
    ...
    render(clutches){
        ...
        {({loading, error, data}) =>{
           ...
           return data.clutches.filter(clutch => clutch[this.state.biltwellstate]
        ).map(({ price, sku, size, helmetType, description, image, brand, name,primaryimage, id,color, helmetColors }) => (
...
export default Biltwell_Helmet_List;
